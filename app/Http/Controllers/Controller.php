...
class Controller extends BaseController
{
    ...
    {
        $campaigns = Campaigns::where('user_id', auth()->user()->id)->get();
    ...
    public function prepareAnalyticsData(PrepareAnalyticsDataRequest $request){
        ...
        ->where('visitors.shop_id', $shop->id)->whereBetween('visitors.timestamp',$startEndDate);
        $cartQuery = $visitorQuery->where('visitors.type', 'add_to_cart');        
        ...
        $orderQuery = Orders::whereHas('codes.campaign_target_history', function($q) use($campaign){
            if($campaign != null) $q->where('campaign_id', $campaign->id);
        })->where($shopWhere)->whereNotNull('browser_ip')->whereBetween(\DB::raw('DATE(`order_date`)'),$startEndDate);       
