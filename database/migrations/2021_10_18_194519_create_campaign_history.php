...
class CreateCampaignHistory extends Migration
{
    ...
    public function up()
    {
        Schema::create('campaign_history', function (Blueprint $table) {
            ...
            $table->bigInteger('count_unique_ip')->unsigned();
            $table->bigInteger('count_mailers_sent')->unsigned();
            $table->float('cost_of_send')->unsigned();
            $table->float('revenue_from_mailers')->unsigned();
            $table->timestamps();
        });
    }
